...
import { MonacoEditorPlaceholder } from "./manacoEditor"
import { RealReactFlowCanvas } from "./graphEditor"
import { RealThreeJsViewer } from "./3dViewer"
  ...
  const resolveContentNode = (view: ViewType, pane: "left" | "right", file = activeFile) => {
    ...
    switch (view) {
      case "editor":
        ...
        else editorContent = <MonacoEditorPlaceholder showChanges={showChanges[pane]} safetyProgram={file?.safety ?? false} fileName={file?.name} statusId={file?.id ?? "editor"} />
        break
      case "flow":
